...
class TestPlaybackControlWorkflow:
    ...
    def test_full_playback_lifecycle(self, mock_msvcrt, sample_audio_bytes):
        ...
        audio_playback.play_audio = Mock(side_effect=lambda audio_bytes: setattr(audio_playback.state, 'is_playing', True) or setattr(audio_playback.state, 'is_paused', False))
        audio_playback.pause = Mock(side_effect=lambda: setattr(audio_playback.state, 'is_paused', True) or setattr(audio_playback.state, 'is_playing', False))
        audio_playback.resume = Mock(side_effect=lambda: setattr(audio_playback.state, 'is_paused', False) or setattr(audio_playback.state, 'is_playing', True))
        ...
        mock_msvcrt.kbhit.side_effect = [
            True, True, True, False  # Three key presses, then stop
        ]
