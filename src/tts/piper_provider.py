...
def _ensure_piper_installed() -> None:
    ...
    try:
        ...
        if result.returncode == 0:
            logger.debug(f"Piper found: {result.stdout.decode().strip()}")
            return
...
def _ensure_model_available(voice: str) -> None:
    ...
    try:
        ...
        logger.warning("Model download not fully implemented; using placeholder")

        # For now, create dummy files to allow testing
...
def _run_piper_synthesis(text: str, voice: str, speed: float) -> bytes:
    ...
    try:
        ...
        raise PiperTTSError("Piper synthesis timed out (>30s)")
    except FileNotFoundError:
