...
class RecordingIndicator:
    ...
    def _animate(self) -> None:
        ...
        while self.is_running:
            ...
            status_parts.append(
                f"{Fore.WHITE}[{device_display}]{Style.RESET_ALL}"
            )
...
class ProgressIndicator:
    ...
    def stop(self, success: bool = True) -> float:
        ...
        print(
            f"{symbol} {self.message} "
            f"{Fore.WHITE}(completed in {elapsed:.1f}s){Style.RESET_ALL}"
        )
...
def format_transcription_result(
    ...
    meta_line2 = f" {Fore.WHITE}Model:{Style.RESET_ALL} {model_name} | {Fore.WHITE}Transcribed:{Style.RESET_ALL} {timestamp}"
    ...
    for line in text_lines:
        ...
        formatted_text.append(
            f"{Fore.CYAN}║{Style.RESET_ALL} {line}{' ' * padding} {Fore.CYAN}║{Style.RESET_ALL}"
        )
...
def format_error_box(
    ...
    if suggestions:
        ...
        formatted_suggestions.append(
            f"{color}║{Style.RESET_ALL} {Fore.CYAN}💡 Try:{Style.RESET_ALL}"
            + " " * 58 + f"{color}║{Style.RESET_ALL}"
        )
...
def format_device_list() -> str:
    ...
    lines.append(
        f"{Fore.WHITE}  ID  │  Device Name                              │  Channels{Style.RESET_ALL}"
    )
    lines.append(f"{Fore.CYAN}{'─' * 70}{Style.RESET_ALL}")
    ...
    for device in devices:
        ...
        lines.append(
            f"{marker} {device['id']:2d}  │  {name:40s}  │  {device['channels']}"
        )
