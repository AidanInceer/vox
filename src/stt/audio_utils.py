...
class SilenceDetector:
    ...
    def __init__(
        ...
        logger.debug(
            f"SilenceDetector initialized: {silence_duration}s = {self.required_chunks} chunks of {chunk_duration}s"
        )
    ...
    def process_chunk(self, audio_chunk: np.ndarray) -> bool:
        ...
        """
        is_silent = detect_silence(audio_chunk, threshold=self.silence_threshold)
        ...
        if is_silent:
            ...
            if self.silent_chunks % 10 == 0:  # Log every 1 second
                logger.debug(f"Silence: {self.silent_chunks}/{self.required_chunks} chunks")
