...
# even when optional runtime dependencies are missing.
try:
    import psutil
except Exception:
    psutil = None
...
def get_browser_tabs(browser_name: str) -> List[TabInfo]:
    ...
    """
    # Allow tests/mocks that patch module attribute `get_browser_tabs` to
    # replace behavior at runtime. If the module attribute has been
    # replaced (e.g., by unittest.mock.patch), delegate to that object so
    # tests that patch the function after importing will still cause the
    # patched callable to be invoked.
    module = sys.modules.get(__name__)
    if module is not None:
        module_attr = getattr(module, "get_browser_tabs", None)
        if module_attr is not None and module_attr is not get_browser_tabs:
            return module_attr(browser_name)
    ...
    try:
        ...
        raise BrowserDetectionError(f"Failed to detect {browser_name} tabs: {str(e)}") from e
...
def _is_process_running(process_names: List[str]) -> bool:
    ...
    """
    # If psutil is not available, assume no processes are running.
    if psutil is None:
        return False
